...
ftimer_gettod (ftimer_test_funct f, void *argp, int n)
{
  ...
  diff = 1E3L * (etv.tv_sec - stv.tv_sec) + 1E-3L * (etv.tv_usec - stv.tv_usec);
  diff /= n;
  return (1E-3 * diff);
}
...
get_etime (void)
{
  ...
  return (long double)((first_p.it_value.tv_sec - r_curr.it_value.tv_sec) +
                  (first_p.it_value.tv_usec - r_curr.it_value.tv_usec) * 1e-6L);
}
